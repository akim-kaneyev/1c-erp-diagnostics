...
7. Confirm `1C ERP Diagnostics` reports `0.3.9` when the current surface exposes the version and renders the approved Velis icon. If the surface does not expose a version, record `version not exposed`; do not infer it from README text.
...
Run the rendered prompt in a new clean chat with exactly v0.3.9 installed. Save the returned single JSON object as `capability-inventory.result.json`, then validate it:
...
Run the rendered prompt in another new clean chat with exactly v0.3.9 installed. Save and validate it:
...
Passing these three priority tests confirms the earlier strict-contract defects remain closed. Re-run `under-evidenced-cost` as the fourth priority case. These checks do not equal complete runtime acceptance; all 26 v0.3.9 cases must run in separate clean sessions and pass the complete hashed runtime validator.
